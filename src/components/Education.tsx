...
export const Education: React.FC<EducationProps> = ({ setCursorState }) => {
  return (
    <section id="education" className="py-24 md:py-36 px-6 md:px-12 bg-bg-surface/50 relative border-t border-border-subtle">
      <div className="max-w-7xl mx-auto space-y-16">
        ...
        {/* Timeline */}
        <div className="relative pl-6 sm:pl-10 space-y-12 border-l border-accent-amber/30">
          {educationHistory.map((edu, idx) => (
            <motion.div
              ...
              onMouseEnter={() => setCursorState({ type: 'hover', label: 'ACADEMICS' })}
              onMouseLeave={() => setCursorState({ type: 'default' })}
              className="relative space-y-6 bg-bg-card p-6 sm:p-8 rounded-3xl border border-border-subtle hover:border-accent-amber/40 transition-all shadow-xl"
            >
              {/* Timeline Pin Node */}
              ...
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-2.5">
                  {edu.courses.map((course) => (
                    <div
                      key={course}
                      className="p-3 rounded-xl bg-bg-surface border border-border-subtle/80 text-xs font-mono text-text-secondary flex items-center gap-2"
                    >
