...
import { MapPin, Calendar, Heart, Navigation, Plus, Globe, ArrowRight, Radio } from 'lucide-react';
...
export const TravelMap: React.FC<TravelMapProps> = ({
  ...
  const [mapMode, setMapMode] = useState<'world' | 'bangladesh'>('bangladesh');
  ...
  return (
    <div className="rounded-3xl bg-bg-surface/90 border border-border-subtle p-6 sm:p-10 shadow-2xl relative overflow-hidden backdrop-blur-md">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6 relative z-20 pb-4 border-b border-border-subtle/80">
        <div className="flex items-center gap-3">
          <div className="p-2.5 rounded-xl bg-accent-amber/10 text-accent-gold border border-accent-amber/20">
            <Radio className="h-5 w-5 animate-pulse text-accent-amber" />
          </div>
          <div>
            <span className="text-[10px] font-mono text-accent-gold uppercase tracking-widest block flex items-center gap-1.5">
              ...
              AUTHENTIC GEOGRAPHIC SATELLITE ENGINE
            </span>
            <h3 className="text-xl font-display font-extrabold text-text-primary">
              {mapMode === 'bangladesh' ? 'Bangladesh Regional Satellite Focus' : 'Global Earth Satellite View'}
            </h3>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2 bg-bg-card p-1.5 rounded-2xl border border-border-subtle">
          ...
          >
            <MapPin className="h-3.5 w-3.5" /> BANGLADESH MAP
          </button>
          ...
          >
            <Globe className="h-3.5 w-3.5" /> WORLD VIEW
          </button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-center relative z-10">
        ...
        {/* ── LEFT: REAL SATELLITE MAP DISPLAY ── */}
        <div className="lg:col-span-6 relative flex flex-col items-center justify-center py-2">
          ...
          <div className="relative w-full max-w-md aspect-[4/5] bg-[#07070a] rounded-2xl border border-border-subtle p-0 flex items-center justify-center shadow-2xl overflow-hidden group">
            ...
            {/* Grid Overlay Lines */}
            <div className="absolute inset-0 bg-[linear-gradient(to_right,#ffffff05_1px,transparent_1px),linear-gradient(to_bottom,#ffffff05_1px,transparent_1px)] bg-[size:24px_24px] pointer-events-none z-10" />
            ...
            <AnimatePresence mode="wait">
              {mapMode === 'bangladesh' ? (
                /* REAL BANGLADESH MAP IMAGE WITH VISITED LOCATION PINS */
                <motion.div
                ...
                >
                  ...
                  <div style={{ top: '48%', left: '68%' }} className="absolute -translate-x-1/2 -translate-y-1/2 z-30">
                  ...
                  <div className="absolute bottom-4 left-4 right-4 bg-black/85 backdrop-blur-md p-3.5 rounded-xl border border-white/15 flex items-center justify-between z-20">
                    <div>
                      <span className="text-[9px] font-mono text-accent-gold uppercase tracking-widest block">SPATIAL CAMERA</span>
                    ...
                    <button
                      ...
                      className="px-3.5 py-2 rounded-lg bg-accent-amber text-bg-primary font-mono text-[10px] font-extrabold tracking-wider hover:bg-amber-300 transition-colors flex items-center gap-1.5 shadow-lg"
                    >
          ...
          <div className="mt-3 text-[11px] font-mono text-text-muted flex items-center justify-between w-full max-w-md px-1">
            <span className="flex items-center gap-1.5">
              ...
              Click location markers on the Bangladesh Map Image
            </span>
            {onOpenAdminModal && (
              <button
